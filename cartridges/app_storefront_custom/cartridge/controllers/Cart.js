...
server.post("AddProduct", function(req, res, next) {
    ...
    var childProducts = Object.hasOwnProperty.call(req.form, "childProducts") ? JSON.parse(req.form.childProducts) : [];
    ...
    if (currentBasket) {
        Transaction.wrap(function() {
            if (!req.form.pidsObj) {
                ...
                result = cartHelper.addProductToCart(currentBasket, productId, quantity, childProducts, options);
            } else {
                // product set
...
server.get("Show", server.middleware.https, consentTracking.consent, csrfProtection.generateToken, function(
    req,
    res,
    next
) {
...
server.get("UpdateQuantity", function(req, res, next) {
        ...
        if (matchingLineItem.product.bundle) {
            ...
            canBeUpdated = collections.every(bundleItems, function(item) {
                var quantityToUpdate =
                    updateQuantity * matchingLineItem.product.getBundledProductQuantity(item.product).value;
    ...
    if (canBeUpdated) {
        Transaction.wrap(function() {
            ...
            var previousBounsDiscountLineItems = collections.map(currentBasket.bonusDiscountLineItems, function(
...
server.get("AddCoupon", server.middleware.https, csrfProtection.validateAjaxRequest, function(req, res, next) {
    var BasketMgr = require("dw/order/BasketMgr");
    var Resource = require("dw/web/Resource");
    var Transaction = require("dw/system/Transaction");
    var URLUtils = require("dw/web/URLUtils");
...
server.get("EditBonusProduct", function(req, res, next) {
    ...
    var selectedBonusProducts = collections.map(bonusDiscountLineItem.bonusProductLineItems, function(
...
server.post("EditProductLineItem", function(req, res, next) {
    ...
    if (requestLineItem) {
        if (requestLineItem.product.bundle) {
            ...
            canBeUpdated = collections.every(bundleItems, function(item) {
                var quantityToUpdate =
                    updateQuantity * requestLineItem.product.getBundledProductQuantity(item.product).value;
                qtyAlreadyInCart = cartHelper.getQtyAlreadyInCart(item.productID, productLineItems, item.UUID);
                totalQtyRequested = quantityToUpdate + qtyAlreadyInCart;
                availableToSell = item.product.availabilityModel.inventoryRecord.ATS.value;
                perpetual = item.product.availabilityModel.inventoryRecord.perpetual;
